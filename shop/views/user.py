...
def getUserInfo(request):
    if request.method == 'GET':
        ...
        if request.GET.get('username'):
            ...
            display_name = requested_user_additional_info.display_name

            if requested_user:

                # Create a dict with the values retrieved from the queried data point
                data = {'id': requested_user.id,
                        'username': requested_user.display_name, 'date_joined': str(requested_user.date_joined), 'last_login': str(requested_user.last_login), 'thumbs_up': requested_user_additional_info.thumbs_up, 'thumbs_down': requested_user_additional_info.thumbs_down, 'image': str(requested_user_additional_info.image)}
